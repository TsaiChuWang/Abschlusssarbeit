...
// gcc ./simple_V9_modified.c inih/ini.c -o ../Ausführung/simple_V9_modified -lcurl -lgsl -lgslcblas -lm
// ../Ausführung/simple_V9_modified c
...
int main(int argc, char *argv[]){
    ...
    if(atoi(argv[1]) == _CODE_TENANT_NUMBER){
    ...
    FILE* data_stored_a = fopen("../Datei/simple_V9/statistic.csv", "w+");
    for(int index = 0; index < tenant_number; index++){
        int count = 0;
        for(int time_stamp = 0;time_stamp<tenants[index].timestamps_length;time_stamp++){
    ...
    if(atoi(argv[1]) == _CODE_NAUGHTY_NUMBER){
        sprintf(command, "python3 ./simple_V9.py 4 %d", atoi(argv[2]));
        system(command);
    }
#ifdef REDUCTION
            config.capacity = 360;
            config.tenant_number = 100;
