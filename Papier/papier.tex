...
\author{%
	Steven S. W. Lee, Robert Smith\textsuperscript{3} and Jane Smith\textsuperscript{1}\thanks{Corresponding author: \href{mailto:jane@smith.com}{jane@smith.com}\\ \textbf{Received:} October 20, 2023, \textbf{Published:} December 14, 2023}
}

% Affiliations are output in the \date{} command
\date{\footnotesize Department of Communication Engineering, National Chung Cheng University, Chiayi, Taiwan}

% Full-width abstract
\renewcommand{\maketitlehookd}{%
	\begin{abstract}
		\noindent We address a network design optimization problem in which the optimal bandwidth allocation must accommodate multiple oblivious traffic flows with known statistical properties. We propose a practical traffic management mechanism that ensures the allocated bandwidth meets optimal performance guarantees. Specifically, our approach maintains a packet loss rate below a given threshold 𝜺, provided that each flow adheres to a predefined mean $\mu$ and an interval [$\mu-\sigma, \mu+\sigma$]. Each traffic flow is guaranteed a minimum bandwidth allocation and operates independently, unaffected by other flows.
	\end{abstract}
}
...
\section{II. Problem Formulation and Solution}
...
\section{Queue, GCRA and Priority Queues}
...
\begin{table} % Full width table (notice the starred environment)
	\caption{Symbols and Their Descriptions Used in the Robust Optimization Model}
	\centering % Horizontally center the table
	\begin{tabular}{L{0.15\linewidth} L{0.85\linewidth} } % Manually specify column alignments with L{}, R{} or C{} and widths as a fixed amount, usually as a proportion of \linewidth
		\toprule
		\multicolumn{2}{c}{ \textbf{Constant}} \\
		\cmidrule(r){1-2}
		Symbol 			& Description \\
		\midrule
		...
		\bottomrule
	\end{tabular}
	\label{tab:robust_optimization_symbols}
\end{table}

\begin{equation}
	\begin{aligned}
		\min \ \textcolor{blue}\alpha &&\\
		& \text{subject to}&&\\
        ...
        & \sum_{e\in E^{OUT}_n } \textcolor{blue}{f_{st}(e)} - \sum_{e\in E^{IN}_n} \textcolor{blue}{f_{st}(e)} =&\begin{cases}
           \mu_{st} &\text{if } n=s \\
           0 &\text{if } n\neq s, n\neq t \\
           -\mu_{st} &\text{if } n=t \\
        \end{cases} \\
	...
	\end{aligned}
	\label{tab:robust_optimization_model}
\end{equation}	

The first constraint stipulates that \textbf{the probability that the traffic on link \(e\) does not exceed its capacity must be greater than the constant \(\eta\)}. In our experiments, we set \(\eta = 0.9\). The second constraint ensures that link utilization does not exceed the maximum allowable utilization. If this constraint were omitted, \(\alpha\) would be unconstrained. The third constraint enforces \textbf{flow conservation}. Since \(d_{st}\) is a random variable, the demand is replaced by its mean, \(\mu_{st}\). The final constraint addresses the \textbf{non-negativity} of the decision variables, ensuring that \(f_{st}(e) \geq 0\).
...
estibulum sodales orci a nisi interdum tristique. In dictum vehicula dui, eget bibendum purus elementum eu. Pellentesque lobortis mattis mauris, non feugiat dolor vulputate a. Cras porttitor dapibus lacus at pulvinar. Praesent eu nunc et libero porttitor malesuada tempus quis massa. Aenean cursus ipsum a velit ultricies sagittis. Sed non leo ullamcorper, suscipit massa ut, pulvinar erat. Aliquam erat volutpat. Nulla non lacus vitae mi placerat tincidunt et ac diam. Aliquam tincidunt augue sem, ut vestibulum est volutpat eget. Suspendisse potenti. Integer condimentum, risus nec maximus elementum, lacus purus porta arcu, at ultrices diam nisl eget urna. Curabitur sollicitudin diam quis sollicitudin varius. Ut porta erat ornare laoreet euismod. In tincidunt purus dui, nec egestas dui convallis non. In vestibulum ipsum in dictum scelerisque.
...
\subsection{Grid Based Packets and Configuration}
...
\subsubsection{Individual Traffic Shaping}
