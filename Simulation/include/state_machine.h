 ...
 */
state_machine* initialize_state_machines(double state_r, double generate_probability, double generate_probability_naughty, int tenant_number, int naughty_tenant_number){
...
void change_state(state_machine* pstate, int random){
  ...
  double factor = drand48();; /**< Generate a random factor between 0 and 1. */

  if (pstate->state == STATE_CREATE_PACKET) {  /**< Check if the current state is STATE_CREATE_PACKET. */
    if (factor < pstate->r) {
      pstate->state = STATE_CREATE_PACKET;  /**< Stay in STATE_CREATE_PACKET. */
    } else {
      pstate->state = STATE_NO_PACKET;  /**< Transition to STATE_NO_PACKET. */
    }
  } else {  /**< If the current state is STATE_NO_PACKET. */
    if (factor < pstate->p) {
