...
IMAGE_PATH = './different_r_{}_{}.png'
DATA_PATH = '/home/wtsaichu/Dokumente/Arbeitsplatz/Abschlusssarbeit/Simulation/python/regular_and_naughty_all.csv'
...
for n in num:
    for key in keys:
        nloss = dataframe[(dataframe['state_r'] == key[0]) & (dataframe['diff'] == key[1]) & (dataframe['naughty_tenant_number'] == n)]['naughty_loss'].tolist()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(IMAGE_PATH.format('naughty', n))
    plt.cla()  

    for key in keys:
        rloss = dataframe[(dataframe['state_r'] == key[0]) & (dataframe['diff'] == key[1]) & (dataframe['naughty_tenant_number'] == n)]['regular_loss'].tolist()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(IMAGE_PATH.format('regular', n))
    plt.cla()  
# if(int(sys.argv[1]) == R):
